...
// 列挙型はデータを保持することもできる。matchを使って取り出したりする
enum Command<'a>{
    Comment(&'a str),
    Argument(&'a str),
    Control(&'a str),
...
fn assemble(program_path: &str){
    ...
    let mut symbol_table = create_symbol_table(&commands);

    let mut wf = get_write_file(&program_path);
    ...
    for command in &commands{
        ...
        if let Some(line) = asm_line {
            write_bit(&mut wf, &line);
            print!("{}", line);
        }
    }
}

fn get_write_file(file_name:&str) -> File{
    // let name = Path::new(file_name).file_name();
    let names = file_name.rsplitn(2, ".").collect::<Vec<&str>>();
    let name = format!("{}.hack",names[1]);
...
fn write_bit(file: &mut File, bit_str: &str){
    file
        .write_all(bit_str.as_bytes())
        .unwrap_or_else(|err| {
...
fn create_symbol_table<'a>(commands: &'a [Command]) -> HashMap<&'a str, u16> {
